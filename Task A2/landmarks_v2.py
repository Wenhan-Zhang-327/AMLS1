...
basedir = 'D:/ELEC0134 Applied Machine Learning Systems/assignment/dataset_AMLS_20-21/'
#images_dir = os.path.join(basedir,'img')
#images_dir = images_dir.replace('\\', '/')
images_dir = pathlib.Path('/').joinpath(basedir,'celeba')
...
def extract_features_labels():
    ...
    :return:
        ...
        gender_labels:      an array containing the gender label (notsmiling=0 and smiling=1) for each image in
...
#             }, ignore_index=True)
    ...
    smile_labels = (np.array(all_labels) + 1)/2 # simply converts the -1 into 0, so notsmiling=0 and smiling=1
